...
- 可配置自动复盘：默认关闭，核验执行结果后可自动调用 DeepSeek，并明确提示 Token 成本
- API Key 仅保留在运行内存中
...
- AI 步骤一键启动：可区分“只设为任务”和“设为任务并立即专注”；仅允许在全新停止阶段执行，若当前是休息阶段会安全切回专注
...
- 可执行截止约束：本地识别绝对时间、今晚/明天/周几和相对时长，并以“手动可用时间与截止剩余时间的较小值”约束 AI 和质量门
- DeepSeek 配置诊断：不发送目标和对话内容即可检查密钥、余额可用状态与所选模型，分别提示常见故障
- DeepSeek 调用审计：展示每次生成、复盘和质量修复使用的模型、耗时、Token 与结果，明确区分远端响应和未确认失败
- 对话级 Token 预算：按服务端 usage 累计消耗，80% 时预警、达到上限后停止新调用，并阻止超预算自动修复
- AI 执行结果核验：达到预计番茄数只代表时间达标，需记录可检查交付、部分进度或具体阻塞后再复盘
- AI 计划安全回退：模型确实改动已有计划时自动保留单级快照，支持不调用 AI 一键恢复上版，并随会话历史持久化
- AI 计划提案确认：模型结果先展示新增、修改、删除和番茄总量变化，支持逐项选择、全部应用或拒绝，不再直接覆盖执行计划
- 轻量专注复盘：每轮有效专注后记录专注度、精力、主要打断和实际产出，并进入历史与 Agent 近期执行上下文
- 沉浸式计时：一键隐藏信息卡与底部导航，只保留任务、倒计时和阶段控制
- HarmonyOS 跨设备接续原型：可在手机、平板和 2in1 设备间迁移当前倒计时、任务、Agent 计划与近期专注记录
- 跨设备冲突保护：记录最近一次已应用快照的时间和传输标识，重复或更旧的接续不会回滚本机状态
- 无真机穿戴协同原型：设置页内置虚拟手表，以滑动窗口模拟平稳、压力、运动和断连，并支持个人基线校准
- 心率自适应安全链路：持续高负荷可进入不消耗原专注时间的两分钟恢复；聚合摘要以受控动作请求进入 Agent，确认后仍不会自动调用 AI
- AI 页面响应式布局：手机保持连续单列，窗口达到 720vp 后自动切换为“控制与上下文 / 对话与执行计划”5:7 双栏
- AI 工作流进度轨道：用连线、完成标记和“当前阶段”提示展示“目标 / 计划 / 执行 / 复盘”的自动推进状态
- AI 计划渐进式界面：首屏聚焦目标、下一步和执行计划，服务设置使用独立弹层，上下文、记忆、用量与调用明细统一按需展开
- 计划概览、质量详情和对话记录默认收起，只有当前下一步显示执行操作，减少重复按钮和长页面滚动
- 历史记录检索与筛选：按任务、项目、标签、产出和打断原因搜索，并支持专注、休息、完成状态与复盘状态筛选
- 历史任务一键重做：在安全的停止初始态恢复任务上下文、项目、标签、备注和估算番茄数，自动返回计时页
- 行动型数据洞察：根据历史记录计算高效时段、估时准确度、平均专注度和主要打断，并生成下一轮可执行建议
- 历史复盘可修正：在可滚动的独立编辑态补充、修改或移除专注度、精力、打断原因和实际产出
- 洞察可信度提示：对比近 7 天与此前 7 天专注量，并展示复盘覆盖率，反馈样本不足时优先提醒继续积累
...
| 完备度 | 4 个清晰主 Tab、数据中心三分区、Agent 记忆、执行核验、调用审计、Token 预算、计划提案确认、可修正复盘、趋势洞察、历史检索与重做、沉浸计时、跨设备接续、虚拟手表、会话恢复、75 项核心单元测试 |
...
6. 查看本地健康度、预计耗时和容量风险；核对 AI 提案中的新增、修改和删除项后，选择应用全部或部分修改。
7. 执行下一步；达到预计番茄数后，进入 AI 页面核验实际产出。填写交付证据、部分进度或阻塞原因后再复盘；部分完成和阻塞步骤不会被误算为已完成。需要自动复盘时，可在服务设置中主动开启。
8. 展开“DeepSeek 调用记录”，可核对模型、耗时、输入/输出 Token、质量门结果以及服务端是否返回 usage。
...
| 执行闭环 | 已完成 | 时间达标与成果完成分离，记录交付证据、部分进度或阻塞原因后再驱动复盘与重排 |
| 可观测性 | 已完成 | DeepSeek 调用记录按会话持久化，区分采用、质检拦截、自动修复和失败，且不保存 API Key 或提示词 |
...
在“设置 > 穿戴协同实验室”可以打开虚拟手表，无需真机即可验证计时控制、滑动心率窗口、个人基线校准、两分钟恢复状态机和受控 Agent 调整入口。恢复阶段会冻结原专注倒计时；模拟数据明确标注 `×30` 加速。Agent 请求会先展示允许动作与保护规则，确认只负责预填，仍需用户主动发送。真实手表接入时只替换 `HeartRateProvider` 数据源。设计边界、隐私策略和真机验收清单见 [穿戴协同原型说明](docs/WEARABLE_PROTOTYPE.md)。
...
- AI 步骤达到预计用量后的核验引导，以及手动任务、跨对话任务不会误推进计划
- AI 计划快照的差异识别、深拷贝与空计划过滤
- 跨设备状态的绝对截止时间保持、数据格式校验与历史幂等合并
- 执行核验的具体性校验、阻塞复盘提示与未完成步骤回退
- DeepSeek 调用记录的 usage 捕获、远端响应判断与最近 20 条上限
- 穿戴滑动窗口的持续负荷、运动干扰、断连、个性化基线校准和聚合摘要边界
- AI 计划提案的差异分类、逐项选择与合并结果
- 专注反馈与原始计时记录的精确关联和评分归一化
- 历史记录的多字段搜索、状态筛选与休息记录过滤
- 高效时段、估时准确度、主要打断和低专注优先建议
- 近 7 天趋势对比、休息记录排除和低复盘覆盖率提醒

```powershell
hvigorw test -p module=entry
```

当前验证结果：`75/75` 单元测试通过，`assembleHap` 构建通过。
...
- [DeepSeek Token 预算说明](docs/AI_TOKEN_BUDGET.md)
- [隐私与数据说明](PRIVACY.md)
- [版本更新记录](CHANGELOG.md)
